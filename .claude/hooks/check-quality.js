...
function main() {
  ...
  if (typescriptModifiedFiles.length > 0) {
    console.log(
      `📁 Checking TypeScript for ${typescriptModifiedFiles.length} modified file(s): ${typescriptModifiedFiles.map((f) => path.basename(f)).join(', ')}`
    )
    ...
    for (const pkg of allPackages) {
      ...
      if (fs.existsSync(tsconfigPath)) {
        ...
        const tscResult = runCommand(`cd ${pkg} && npx tsc --noEmit --skipLibCheck`, `TypeScript compilation in ${pkg}`)
  ...
  if (modifiedFiles && modifiedFiles.length > 0) {
    ...
    const eslintResult = runCommand(`npx eslint ${fileList} --max-warnings 0`, `ESLint on modified files`)
