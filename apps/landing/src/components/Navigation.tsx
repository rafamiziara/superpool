  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${hidden ? '-translate-y-full' : 'translate-y-0'} ${
        scrolled ? 'border-b border-hairline-soft bg-abyss/80 backdrop-blur-xl' : 'bg-transparent'
      }`}
    >
      <nav className="mx-auto flex h-18 max-w-6xl items-center justify-between px-5 sm:px-8" aria-label="Main">
        <a href="#top" className="flex items-center gap-2">
          <Image src="/images/logos/no_bg_white.svg" alt="SuperPool" width={140} height={32} className="h-7 w-auto" priority />
        </a>

        <div className="hidden items-center gap-8 md:flex">
          {links.map((link) => (
            <a key={link.href} href={link.href} className="text-sm font-medium text-mist transition-colors duration-300 hover:text-foam">
              {link.label}
            </a>
          ))}
        </div>
