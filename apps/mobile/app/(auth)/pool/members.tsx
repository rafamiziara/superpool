...
function MembersScreen() {
  ...
  */
  ...
  const waiting = pool ? poolStore.getState().pendingMembersFor(pool.poolId) : []
  ...
  const members = pool
    ? poolStore
        .getState()
        .memberRecords.filter((member) => member.poolId === pool.poolId && member.status === 'active')
        .map((member) => ({
          account: member.account,
          balance: poolStore
            .getState()
            .memberships()
            .find((derived) => derived.poolId === String(pool.poolId) && sameAddress(derived.walletAddress, member.account))
            ?.currentBalance,
        }))
    : []
