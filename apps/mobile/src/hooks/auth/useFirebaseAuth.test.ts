...
describe('useFirebaseAuth', () => {
  ...
  it('should handle Firebase auth state changes without creating fake user data', async () => {
    let authStateCallback: (user: { uid: string; displayName?: string | null }) => void
    ...
    // Simulate Firebase auth state change to authenticated
    const mockFirebaseUser = {
      uid: 'firebase-user-id',
      displayName: '0xstatechange',
    }
    ...
    // Should not create fake user data, just log authentication
    expect(result.current.user).toBe(null) // No user data until proper authentication
    expect(result.current.isAuthenticating).toBe(false)
    expect(result.current.error).toBe(null)

    expect(mockConsoleLog).toHaveBeenCalledWith('🔥 Firebase auth state: User authenticated', 'firebase-user-id')
  })
