...
export class BlockchainTestEnvironment {
  ...
  async setupContracts(): Promise<void> {
    ...
    this.poolFactory = new Contract(
      poolFactoryAddress,
      await this.loadContractABI('PoolFactory'),
      this.wallet
    )
    
    if (safeAddress) {
      this.safeContract = new Contract(
        safeAddress,
        await this.loadContractABI('Safe'),
        this.wallet
      )
    }
  }
...
export class ContractTester {
  ...
  async createPool(params: PoolCreationParams): Promise<TransactionResult> {
    ...
    // Estimate gas
    const estimatedGas = await this.blockchain.estimateContractGas(
      this.blockchain.poolFactory,
      'createPool',
      [poolOwner, maxLoanAmountWei, interestRate, loanDuration, name]
    )
...
describe('Pool Creation Contract Integration', () => {
  ...
  describe('Contract Validation', () => {
    it('should reject invalid pool owner address', async () => {
      ...
      // Act & Assert
      await expect(contractTester.createPool(invalidParams))
        .rejects.toThrow(/invalid address|zero address/i)
    })
    ...
    it('should reject zero max loan amount', async () => {
      ...
      // Act & Assert
      await expect(contractTester.createPool(invalidParams))
        .rejects.toThrow(/amount|value/i)
    })
    ...
    it('should reject invalid interest rate', async () => {
      ...
      // Act & Assert
      await expect(contractTester.createPool(invalidParams))
        .rejects.toThrow(/rate|interest/i)
    })
    ...
    it('should reject invalid loan duration', async () => {
      ...
      // Act & Assert
      await expect(contractTester.createPool(invalidParams))
        .rejects.toThrow(/duration|time/i)
    })
  })
...
describe('Safe Multi-Sig Pool Creation Integration', () => {
  ...
  describe('Safe Transaction Validation', () => {
    ...
    it('should validate user is Safe owner before creating transaction', async () => {
      ...
      // Act & Assert
      await expect(createPoolSafeHandler(request))
        .rejects.toThrow(/not a Safe owner|unauthorized|permission denied/i)
    })
  })
  ...
  describe('Transaction Signature Collection', () => {
    it('should collect signatures from multiple Safe owners', async () => {
      ...
      for (let i = 0; i < 3; i++) { // Need 3 signatures for 3-of-5 multisig
    ...
    it('should track signature status correctly', async () => {
      ...
      firebaseAdminMock.firestore.collection('safe_transactions').doc().get.mockResolvedValue({
        exists: true,
...
describe('Pool Event Listener Integration', () => {
  ...
  describe('Event Filtering and Pagination', () => {
    it('should filter events by block range', async () => {
      ...
      const poolPromises = []
      for (let i = 0; i < 3; i++) {
        poolPromises.push(contractTester.createPool({
          poolOwner: '0x742d35Cc6670C74288C2e768dC1E574a0B7DbE7a',
          maxLoanAmount: '1000',
      ...
      const events = await contractTester.blockchain.poolFactory.queryFilter(
        filter,
        startBlockNumber,
        endBlockNumber
      )
  ...
  describe('Event Deduplication', () => {
    it('should handle duplicate events correctly', async () => {
      ...
      // Mock existing event
      firebaseAdminMock.firestore.collection('event_logs').doc(eventId).get.mockResolvedValue({
        exists: true,
        data: () => eventData,
      })
