...
describe('generateAuthMessage Cloud Function', () => {
  ...
  describe('Successful Authentication Flow', () => {
    it('should generate auth message for valid wallet address', async () => {
      ...
      // Act
      const result = await functionTester.expectFunctionSuccess(
  ...
  describe('Nonce Management', () => {
    it('should create unique nonces for concurrent requests', async () => {
      ...
      const [result1, result2] = await Promise.all([
        generateAuthMessageHandler(request1),
        generateAuthMessageHandler(request2),
      ])
...
describe('verifySignatureAndLogin Cloud Function', () => {
  ...
  describe('Successful Authentication', () => {
    it('should verify valid signature and create user session', async () => {
      ...
      firebaseAdminMock.firestore.collection('auth_nonces').where().get.mockResolvedValue({
        empty: false,
        docs: [{
          id: 'nonce-doc-id',
          data: () => ({
      ...
      expect(firebaseAdminMock.auth.createCustomToken).toHaveBeenCalledWith(
    ...
    it('should handle existing user authentication', async () => {
      ...
      // Mock existing user
      firebaseAdminMock.firestore.collection('users').where().get.mockResolvedValue({
        empty: false,
        docs: [{
          id: 'existing-user-id',
          data: () => ({
      ...
      expect(firebaseAdminMock.firestore.collection('users').doc().update || 
             firebaseAdminMock.firestore.collection('users').doc().set).toHaveBeenCalled()
    })
  })
