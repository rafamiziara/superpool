...
export class ContractMock {
  ...
  static createPoolFactoryMock(contractAddress?: string): jest.Mocked<Contract> {
    ...
    const poolFactoryMock = {
      ...
      pools: jest.fn().mockImplementation(async (poolId: bigint | number) => {
      ...
      // Write methods
      createPool: jest.fn().mockImplementation(async (
        poolOwner: string,
        maxLoanAmount: bigint,
        interestRate: number,
        loanDuration: number,
      ...
      transferOwnership: jest.fn().mockImplementation(async (newOwner: string) => {
    ...
    poolFactoryMock.estimateGas.createPool = jest.fn().mockResolvedValue(BigInt('500000'))
    poolFactoryMock.estimateGas.transferOwnership = jest.fn().mockResolvedValue(BigInt('50000'))
    poolFactoryMock.estimateGas.acceptOwnership = jest.fn().mockResolvedValue(BigInt('30000'))
    poolFactoryMock.estimateGas.pause = jest.fn().mockResolvedValue(BigInt('30000'))
    poolFactoryMock.estimateGas.unpause = jest.fn().mockResolvedValue(BigInt('30000'))
    
    // Dynamically populate staticCall methods (for view functions)
    poolFactoryMock.staticCall.pools = poolFactoryMock.pools
    poolFactoryMock.staticCall.poolCount = poolFactoryMock.poolCount
    poolFactoryMock.staticCall.owner = poolFactoryMock.owner
    poolFactoryMock.staticCall.paused = poolFactoryMock.paused
  ...
  static createSafeMock(contractAddress?: string): jest.Mocked<Contract> {
    ...
    const safeMock = {
      ...
      getTransactionHash: jest.fn().mockImplementation(async (
        to: string,
        value: bigint,
        data: string,
        operation: number,
        ...
        const txData = {
          to, value: value.toString(), data, operation,
          safeTxGas: safeTxGas.toString(), baseGas: baseGas.toString(),
          gasPrice: gasPrice.toString(), gasToken, refundReceiver,
      ...
      execTransaction: jest.fn().mockImplementation(async (
        to: string,
        value: bigint,
        data: string,
        operation: number,
            ...
            logs: [
              {
                address: address,
                topics: [
                  '0x' + 'executionfrommodulesuccess'.padEnd(64, '0'),
                ],
