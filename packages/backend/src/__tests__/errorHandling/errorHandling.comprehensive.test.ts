      ...
      it('should handle Firestore rate limiting', async () => {
        await withTestIsolation('rate-limiting', 'error-handling', async (context: TestEnvironmentContext) => {
          ...
          const requests = Array.from({ length: 10 }, async (_, i) => {
            ...
            return ErrorTestServices.authentication.generateAuthMessage(TestFixtures.TestData.addresses.poolOwners[0])
          })
    ...
    describe('Firebase Auth Error Handling', () => {
      it('should handle expired authentication tokens', async () => {
        await withTestIsolation('expired-auth-tokens', 'error-handling', async (context: TestEnvironmentContext) => {
          ...
          const expiredTokenScenarios = ['expired-token-12345', 'revoked-token-67890', 'malformed-token-invalid']
          ...
          for (const scenario of revertScenarios) {
            ErrorTestServices.contracts.executeTransaction.mockImplementation(async () => ({
              ...
              suggestions: scenario.fixable
                ? ['Check function parameters', 'Verify account permissions', 'Ensure sufficient balance']
