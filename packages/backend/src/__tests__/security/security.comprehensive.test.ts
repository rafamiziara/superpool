    ...
    describe('Signature Verification Security', () => {
      it('should prevent signature replay attacks', async () => {
        await withTestIsolation('signature-replay-prevention', 'security', async (context: TestEnvironmentContext) => {
          ...
          const firstVerification = await SecurityServices.authentication.verifySignature(walletAddress, message, signature, nonce)

          // Second verification with same nonce - should fail
          const replayAttempt = await SecurityServices.authentication.verifySignature(walletAddress, message, signature, nonce)
      ...
      it('should detect timing attacks on signature verification', async () => {
        await withTestIsolation('timing-attack-detection', 'security', async (context: TestEnvironmentContext) => {
          ...
          for (const invalidSig of invalidSignatures) {
            await SecurityServices.authentication.verifySignature(TestFixtures.TestData.addresses.poolOwners[0], 'test message', invalidSig)
          }
    ...
    it('should verify data integrity using cryptographic hashes', async () => {
      await withTestIsolation('data-integrity-verification', 'security', async (context: TestEnvironmentContext) => {
        ...
        for (const test of testData) {
          ...
          const tamperedVerification = await SecurityServices.cryptography.verifyIntegrity(test.data + '_tampered', hashResult.hash)
      ...
      await withTestIsolation('security-memory-leaks', 'security', async (context: TestEnvironmentContext) => {
        ...
        const memoryLeakReport = await detectMemoryLeaks(
          ...
          async () => {
            ...
            await SecurityServices.validation.validateEthereumAddress(TestFixtures.TestData.addresses.poolOwners[0])
          },
          operationCount
        )
