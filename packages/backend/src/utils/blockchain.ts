...
export async function waitForConfirmation(
  ...
  try {
    ...
    throw new AppError(`Transaction confirmation failed: ${error instanceof Error ? error.message : String(error)}`, 'CONFIRMATION_FAILED')
  }
}
 ...
 */
export async function isTransactionMined(provider: ethers.Provider, txHash: string): Promise<boolean> {
  try {
 ...
 */
export function parseEventLogs(contract: ethers.Contract, logs: ethers.Log[], eventName: string): ethers.LogDescription[] {
 ...
 */
export async function validateContractAddress(provider: ethers.Provider, address: string): Promise<boolean> {
  try {
