...
class DevEnvironment {
  ...
  async startFirebaseEmulators() {
    ...
    // Pin the project explicitly: without --project the emulator falls back to
    // `firebase use`'s active project, and with singleProjectMode (config/firebase.json)
    // it routes ALL client requests to that one project. If `firebase use` is on the
    // wrong project, local dev silently hits the wrong namespace — wrong token aud,
    // data under the wrong project, permission errors.
    const firebaseProc = this.spawnProcess(
      'firebase',
      ['emulators:start', '--config', './config/firebase.json', '--project', 'genesis-super-pool'],
      {
        cwd: process.cwd(),
      }
    )
  ...
  async updateEnvironmentFile() {
    ...
    // Update ngrok URLs
    envContent = envContent.replace(/EXPO_PUBLIC_NGROK_URL_AUTH="[^"]*"/, `EXPO_PUBLIC_NGROK_URL_AUTH="https://${this.ngrokUrls.auth}"`)
